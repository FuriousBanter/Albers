//tasks: figure out how to move new drawn shapes & blend shapes
...
function setup() {
    ...
    folder1.addColor(optionsMenu, "color").listen();
    folder1.addColor(optionsMenu, "color2").listen();
...
mouseReleased = (mous) => {
    ...
    if ((Math.floor(mouseY) < 200) && (Math.floor(mouseX) > 0) && (Math.floor(mouseX) < 500)) {
        if ((Math.floor(mouseX) < 50) && (Math.floor(mouseX) > 0)) {
            colorPick = color(boxes[0][0].color);
        }
        if ((Math.floor(mouseX) < 100) && (Math.floor(mouseX) > 50)) {
            colorPick = color(boxes[0][1].color);
        }
        if ((Math.floor(mouseX) < 150) && (Math.floor(mouseX) > 100)) {
            colorPick = color(boxes[0][2].color);
        }
        if ((Math.floor(mouseX) < 200) && (Math.floor(mouseX) > 150)) {
            colorPick = color(boxes[0][3].color);
        }
        if ((Math.floor(mouseX) < 250) && (Math.floor(mouseX) > 200)) {
            colorPick = color(boxes[0][4].color);
        }
        if ((Math.floor(mouseX) < 300) && (Math.floor(mouseX) > 250)) {
            colorPick = color(boxes[0][5].color);
        }
        if ((Math.floor(mouseX) < 350) && (Math.floor(mouseX) > 300)) {
            colorPick = color(boxes[0][6].color);
        }
        if ((Math.floor(mouseX) < 400) && (Math.floor(mouseX) > 350)) {
            colorPick = color(boxes[0][7].color);
        }
        console.log("colorPick: " + colorPick);
        // console.log("mouse: " + mouse)
        if (mous.which == 1) {
            // optionsMenu.color.updateDisplay(colorPick);
            optionsMenu.color = colorPick;

            // console.log("1");
        } else if (mous.which == 3) {
            // optionsMenu.color2.updateDisplay(colorPick);
            optionsMenu.color2 = colorPick;

            // console.log("3");
        }
        // f1color1.updateDisplay();
        // f1color2.updateDisplay();
        // gui.updateDisplay();
        console.log(optionsMenu.color);
    }
    // console.log("jaeoip     " + boxes[0][0].color);
